...
int poll(struct pollfd fds[], nfds_t nfds, int timeout_ms)
{
	...
	if (!co_is_enable_sys_hook() || timeout_ms == 0) {  // timeout_ms不为0, 一定会走下面的调用
		return g_sys_poll_func(fds, nfds, timeout_ms);
	}
	// 优化
	...
	if (nfds_merge == nfds || nfds == 1) {
		ret = co_poll_inner(co_get_epoll_ct(), fds, nfds, timeout_ms, g_sys_poll_func);
	} else {
		ret = co_poll_inner(co_get_epoll_ct(), fds_merge, nfds_merge, timeout_ms,
				g_sys_poll_func);
		if (ret > 0) {
			for (size_t i = 0; i < nfds; i++) {
				it = m.find(fds[i].fd);
